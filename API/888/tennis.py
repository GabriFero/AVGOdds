...
#TENNIS
tot_tennis_match = 'https://eu-offering-api.kambicdn.com/offering/v2018/888it/listView/tennis/all/all/all.json?lang=it_IT&market=IT&useCombined=true&useCombinedLive=true'



def get_matchT(headers):
    response = clt.get(url=tot_tennis_match, headers=headers)
...
async def fetch_oddsT(session, Id, headers):
    try:
        ...
        print(f"Errore durante l'elaborazione di tennis_id {Id}: {e}")
        return Id, []


async def get_oddsT(ids, headers):
    all_extracted_odds = {}

    # Creare una sessione aiohttp per le richieste
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_oddsT(session, Id, headers) for Id in ids]  # Creare un task per ogni richiesta
