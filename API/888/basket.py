...
async def fetch_oddsB(session, Id, headers):
    try:
...
async def get_oddsB(ids, headers):
    all_extracted_odds = {}

    # Creare una sessione aiohttp per le richieste
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_oddsB(session, Id, headers) for Id in ids]  # Creare un task per ogni richiesta
