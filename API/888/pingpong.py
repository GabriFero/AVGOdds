...
async def fetch_oddsP(session, Id, headers):
    try:
        ...
        print(f"Errore durante l'elaborazione di pingpong_id {Id}: {e}")
        return Id, []
    

async def get_oddsP(ids, headers):
    all_extracted_odds = {}

    # Creare una sessione aiohttp per le richieste
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_oddsP(session, Id, headers) for Id in ids]  # Creare un task per ogni richiesta
